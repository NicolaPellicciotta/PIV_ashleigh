...
dummyhgui=getappdata(0,'hgui');
dummy=getappdata(dummyhgui);
firstframefilename = dummy.filepath{1};
[dummy, ~,ei] = regexp(firstframefilename, '\d{2}\w{3}\d{4}_\d{2}\.\d{2}\.\d{2}','match')
SuggestedSaveName = [firstframefilename(1:ei),'_PIVlab_session.mat'];
[FileName,PathName] = uiputfile('*.mat','Save current session as...',fullfile(sessionpath,SuggestedSaveName));
if isequal(FileName,0) | isequal(PathName,0)
else
...
app.firstframe = imread(app.filepath{1});
